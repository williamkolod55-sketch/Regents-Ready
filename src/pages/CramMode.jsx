...
export default function CramMode() {
  ...
  if (deck.length === 0 && cleared === 0) {
    return (
      <div className="page-content min-h-screen flex flex-col items-center justify-center px-4 lg:px-10 pb-28 lg:pb-10 gap-4 text-center max-w-lg mx-auto lg:max-w-2xl">
  ...
  if (deck.length === 0 && cleared > 0) {
    return (
      <div className="page-content min-h-screen flex flex-col items-center justify-center px-4 lg:px-10 pb-28 lg:pb-10 gap-5 text-center max-w-lg mx-auto lg:max-w-2xl">
        <motion.div
    ...
    <div className="page-content min-h-screen flex flex-col px-4 lg:px-10 pt-8 pb-28 lg:pb-10 max-w-lg mx-auto lg:max-w-2xl">
      ...
      <div className="flex items-center justify-between mb-4">
        ...
        <div className="text-center">
          <div className="text-sm font-black">Cram Mode 🔥</div>
          <div className="text-xs text-orange-400 font-semibold">{deck.length} cards left</div>
        </div>
        <div className="text-sm font-bold text-green-400">{cleared > 0 ? `+${cleared}` : ''}</div>
      </div>
      ...
      <div className="text-xs text-white/30 text-center mb-4">Sorted by miss count — most missed first</div>

      {/* Flip Card */}
      <div className="flex-1 flex flex-col items-center justify-center">
        <div className="w-full card-3d-container" style={{ height: 340 }}>
              ...
              >
                ...
                >
                  ...
                  <div className="card-face card-face-back glass flex flex-col justify-center p-6"
                    ...
                    <h3 className="text-base font-bold text-white/70 mb-3">{currentCard.term}</h3>
                    <ul className="space-y-2.5">
                      {currentCard.bullets.map((bullet, i) => (
                        <li key={i} className="flex items-start gap-2.5">
                          <span className="mt-1 w-1.5 h-1.5 rounded-full flex-shrink-0 bg-orange-400" />
                          <span className="text-[15px] leading-snug text-white/85">{bullet}</span>
                        </li>
                      ))}
                    </ul>
                  </div>
